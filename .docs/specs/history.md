...
- 変更日時: 2026-01-14
- 変更内容の概要
  - UI の非アクティブ化を検知し、自動で非表示にする
  - UI スレッド ID を UI 側から通知し、終了要求で WM_QUIT を送れるようにする
  - UI ウィンドウにタスクバー非表示のスタイルを適用する
- 変更理由

  - 入力が外れた際に UI を自動で畳み、タスクバーに常駐表示されないようにするため
...
- 変更日時: 2026-01-14
- 変更内容の概要
  - タスクトレイを起動経路に組み込み、別スレッドで常駐イベントループを実行する方式へ更新
  - タスクトレイと Controller 間の通信を双方向に整理し、終了指示を送れるように更新
  - UI 側にフォーカス要求コマンドを追加し、表示時に入力欄へフォーカスを戻す拡張のための経路を追加
  - タスクトレイアイコンをビルド時にデコードして埋め込む方式に更新（build.rs で RGBA データを生成し include! で取り込む）
  - ホットキーおよび Controller ループに短いスリープを追加し、ポーリングによる CPU 負荷を抑える
- 変更理由

  - タスクトレイ機能を Controller 中心の構成に統合し、表示と終了の導線を安定させるため
